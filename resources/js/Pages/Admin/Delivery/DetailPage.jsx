...
const DetailPage = ({ show, onCancel, payload }) => {
    ...
    const details = [
        ...
        { icon: <Barcode />, label: "SO CODE", value: payload.sales_order?.code || '-' },
    ...
    const delivery_items = payload.items || [];

    const calculateSubtotal = () => {
        return delivery_items.reduce((total, item) => total + (parseFloat(item.subtotal) || 0), 0);
    };
                ...
                <Card className="h-fit">
                    ...
                    <CardContent className="space-y-4">
                        {details.map((item, index) => (
                            <div key={index} className={`flex items-start gap-3 border-b pb-3 last:border-b-0 ${!item.value ? 'hidden' : ''}`}>
