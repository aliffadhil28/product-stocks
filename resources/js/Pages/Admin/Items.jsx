...
import FormPage from './Items/FormPage';
import {
    Card,
    CardContent,
    CardDescription,
...
const Items = () => {
    ...
    const handleDelete = async (itemId) => {
        try {
            const response = await fetchPost('MasterItemController', 'destroy', { id: itemId });
            if (response) {
    ...
    const handleSubmit = async (formData) => {
        try {
            if (mode === 'add') {
                const response = await fetchPost('MasterItemController', 'store', formData);
                if (response) {
                ...
                const response = await fetchPost('MasterItemController', 'update', { id: payload.id, ...formData });
                if (response) {
                    fetchItems(); // Refresh the table
                    handleCancel();
    ...
    const fetchItems = async () => {
        setIsLoading(true);
        try {
            const response = await fetchPost('MasterItemController', 'index', {});
            if (response) {
            ...
            columnHelper.display({
                ...
                cell: info => (
                    <div className="flex gap-2">
                        {(hasPermission('items/edit') || hasRole('admin')) && (
                            <button
                        ...
                        {(hasPermission('items/delete') || hasRole('admin')) && (
                            <>
                                <AlertDialog>
    ...
    return (
        <DashboardLayout header="Master Items">
            <div className="py-6">
                {isIndexShown && (
                    <Card className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <CardHeader className="flex justify-between items-center flex-row">
                            ...
                            <button
                                style={{ display: hasPermission('items/create') || hasRole('admin') ? 'inline-flex' : 'none' }}
                                type="button"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="text-sm text-gray-500">
                                            Showing {table.getPrePaginationRowModel().rows.length} of {items.length} items
                                        </div>
                                    </div>
