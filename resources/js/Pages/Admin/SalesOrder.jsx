...
import FormPage from './SalesOrder/FormPage';
import {
    Card,
    CardContent,
    CardDescription,
...
const SalesOrder = () => {
    ...
    const controller = "SalesOrderController";
            ...
            columnHelper.display({
                ...
                cell: info => (
                    <div className="flex gap-2">
                        {(hasPermission('sales-order/view') || hasRole('admin')) && (
                            <button
                        ...
                        {(hasPermission('sales-order/edit') || hasRole('admin')) && (info.row.original.status !== 'approved' && info.row.original.status !== 'rejected') && (
                            <button
                        ...
                        {(hasPermission('sales-order/delete') || hasRole('admin')) && (info.row.original.status === 'rejected') && (
                            <>
                                <AlertDialog>
    ...
    return (
        <DashboardLayout header="Sales Orders">
            <div className="py-6">
                {isIndexShown && (
                    <Card className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <CardHeader className="flex justify-between items-center flex-row">
                            ...
                            <button
                                style={{ display: hasPermission('sales-order/create') || hasRole('admin') ? 'inline-flex' : 'none' }}
                                type="button"
