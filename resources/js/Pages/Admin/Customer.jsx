...
import DetailPage from './Customer/DetailPage';
...
const Customer = () => {
    ...
    const handleEdit = (customer) => {
        setIsIndexShown(false);
        setMode('edit');
        // Pass customer data to FormPage
        // You might need to set selected customer state here
        setPayload(customer);
    }

    const handleDelete = async (customerId) => {
        try {
            const response = await fetchPost('MasterCustomerController', 'destroy', { id: customerId });
            if (response) {
                // Remove customer from state
                setCustomers(customers.filter(customer => customer.id !== customerId));
                // Or refetch customers
                fetchCustomers();
    ...
    const handleSubmit = async (formData) => {
        try {
            if (mode === 'add') {
                const response = await fetchPost('MasterCustomerController', 'store', formData);
                if (response) {
                ...
                const response = await fetchPost('MasterCustomerController', 'update', { id: payload.id, ...formData });
                if (response) {
                    fetchCustomers(); // Refresh the table
                    handleCancel();
    ...
    const fetchCustomers = async () => {
        setIsLoading(true);
        try {
            const response = await fetchPost('MasterCustomerController', 'index', {});
            if (response) {
            ...
            columnHelper.display({
                ...
                cell: info => (
                    <div className="flex gap-2">
                        {(hasPermission('customer/view') || hasRole('admin')) && (
                            <button
                        ...
                        {(hasPermission('customer/edit') || hasRole('admin')) && (
                            <button
                        ...
                        {(hasPermission('customer/delete') || hasRole('admin')) && (
                            <>
                                <AlertDialog>
    ...
    return (
        <DashboardLayout header="Master Customers">
            <div className="py-6">
                {isIndexShown && (
                    <Card className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <CardHeader className="flex justify-between items-center flex-row">
                            ...
                            <button
                                style={{ display: hasPermission('customer/create') || hasRole('admin') ? 'inline-flex' : 'none' }}
                                type="button"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="text-sm text-gray-500">
                                            Showing {table.getPrePaginationRowModel().rows.length} of {customers.length} customers
                                        </div>
                                    </div>
