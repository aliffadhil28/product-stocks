...
import FormPage from './PurchaseOrder/FormPage';
import {
    Card,
    CardContent,
    CardDescription,
...
import DetailPage from './PurchaseOrder/DetailPage';
...
const PurchaseOrder = () => {
    ...
    const controller = "PurchaseOrderController";
    ...
    const handleDelete = async (orderId) => {
        try {
            const response = await fetchPost(controller, 'destroy', { id: orderId });
            if (response) {
            ...
            columnHelper.display({
                ...
                cell: info => (
                    <div className="flex gap-2">
                        {(hasPermission('purchase-order/view') || hasRole('admin')) && (
                            <button
                        ...
                        {(hasPermission('purchase-order/edit') || hasRole('admin')) && (info.row.original.status !== 'approved' && info.row.original.status !== 'rejected') && (
                            <button
                        ...
                        {(hasPermission('purchase-order/delete') || hasRole('admin')) && (info.row.original.status === 'rejected') && (
                            <>
                                <AlertDialog>
    ...
    return (
        <DashboardLayout header="Purchase Orders">
            <div className="py-6">
                {isIndexShown && (
                    <Card className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <CardHeader className="flex justify-between items-center flex-row">
                            ...
                            <button
                                style={{ display: hasPermission('purchase-order/create') || hasRole('admin') ? 'inline-flex' : 'none' }}
                                type="button"
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="text-sm text-gray-500">
                                            Showing {table.getPrePaginationRowModel().rows.length} of {orders.length} items
                                        </div>
                                    </div>
